...
const records = [];
const pending = [];
for (const slug of fs.readdirSync("data/songs")) {
  const file = `data/songs/${slug}/song.json`;
  ...
  if (!entry || norm(entry.musicTitle[0]) !== norm(song.title))
...
fs.writeFileSync(
  "data/research/song-timing.json",
  JSON.stringify(report, null, 2) + "\n",
);
if (process.argv.includes("--apply")) {
  for (const { file, song, timing } of pending) {
    for (const [key, value] of Object.entries(timing)) {
      if (song[key] != null && song[key] !== value)
        throw new Error(`${file}: 既存の${key}を上書きしません。`);
    }
  }
  for (const { file, song, timing } of pending)
    fs.writeFileSync(
      file,
      JSON.stringify({ ...song, ...timing }, null, 2) + "\n",
    );
}
console.log(
  JSON.stringify(
