...
同じ曲の別譜面や別ゲームの収録先を指定する場合は「同じ楽曲の別の譜面・収録先」に `garupa:667` や `ournotes:7` のように1行1曲で入力します。相手のIDは各ゲームの管理画面の一覧か、公開楽曲ページのURLで確認できます。保存すると相手の曲データも同じGitHubコミットで更新され、双方の楽曲ページにリンクが出ます。削除する場合はその行を消して保存します。既に曲名と作曲者が一致して自動表示されるリンクは、欄が空でも引き続き表示されます。
