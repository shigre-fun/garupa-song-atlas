...
楽曲フォームには基本BPM・BPMの下限/上限・ゲーム内演奏時間（秒）の欄があります。一定なら基本BPMだけ、変動する場合は下限と上限も両方入力します。小数にも対応しています。1分45.384秒なら105.384と入力してください。未確認は空欄にでき、詳細ページでは「未確認」と表示されます。既存曲を読み込むとこれらの値も復元されます。
