...
同じ曲の別譜面や別ゲームの収録先を指定する場合は「+ 別の楽曲ページのリンクを追加する」を押します。ガルパ・アワーノーツの曲一覧が開くので、ゲームを選び、曲名・読み・バンド名で検索して対象の曲を追加してください。IDの手入力は不要です。公開楽曲ページにある既存のリンクも曲名で表示され、「削除」で外せます。保存すると相手の曲データも同じGitHubコミットで更新され、双方の楽曲ページのリンクが更新されます。
