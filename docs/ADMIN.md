...
4. 「楽曲を保存する」を押します。入力内容はGitHubの `data/garupa/songs.json` の該当グループに追加され、管理用IDも同じコミットで更新されます。
...
公開確認リンクは曲名フォルダーではなく `/garupa/songs/{管理ID}/` を開きます。曲名を修正しても恒久URLは変わりません。独自ドメインに移るとブラウザー内の下書きは自動で移行しないため、必要な内容は事前に控えてください。[公開先の移行手順](DEPLOYMENT.md)も参照してください。
...
管理用番号は `data/garupa/admin-state.json` の `nextId` で採番し、`data/garupa/songs.json` と同じコミットで更新します。既存の追加コマンドもこの番号を更新します。
