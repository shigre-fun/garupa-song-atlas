...
2. 作曲者、原曲アーティスト、作品名、ガルパの3Dライブ対応状況、アワーノーツのMV有無などを入力します。未確認の任意情報は空欄のままでも保存できます。
...
同じ曲の別譜面や別ゲームの収録先を指定する場合は「+ 別の楽曲ページのリンクを追加する」を押します。ガルパ・アワーノーツの曲一覧が開くので、ゲームを選び、曲名・読み・バンド名で検索して対象の曲を追加してください。IDの手入力は不要です。追加済みのリンクは曲名で確認でき、「削除」で外せます。保存すると相手の曲データも同じGitHubコミットで更新され、双方の楽曲ページにリンクが出ます。既に曲名と作曲者が一致して自動表示されるリンクは、欄が空でも引き続き表示されます。
